...
function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('translationDirection')
  ...
  const toggleDirection = useCallback(() => {
    setDirection(prev => {
      const next = prev === 'en-zh' ? 'zh-en' : 'en-zh'
  ...
  const handleTranslate = () => translateText(input, direction)

  const handleSpeechResult = useCallback(async (text) => {
    if (!text?.trim()) return
    setInput(text)
    const lang = detectLanguage(text)
    const dir = lang === 'zh' ? 'zh-en' : 'en-zh'
    setDirection(dir)
    localStorage.setItem('translationDirection', dir)
    await translateText(text, dir)
  }, [translateText])
  ...
  return (
    <div className="app">
      ...
      <LanguageSwitch direction={direction} onToggle={toggleDirection} />

      <div className="translator-container">
        <div className="input-methods">
          <VoiceButton onSpeechResult={handleSpeechResult} isLoading={loading} />
          <button className="camera-trigger-btn" onClick={() => setShowCamera(true)}>
            📷 Capture Image
          </button>
        </div>
        ...
        <div className="input-section">
          ...
          <div className="button-group">
            <button className="translate-btn" onClick={handleTranslate} disabled={loading || !input.trim()}>
        ...
        <div className="output-section">
          ...
          {output && (
            <div className="output-actions">
              <button className="action-btn" onClick={speakTranslation}>🔊 Listen</button>
              <button className="action-btn" onClick={() => navigator.clipboard.writeText(output)}>📋 Copy</button>
            </div>
          )}
        </div>
      </div>
